...
var DEFAULT_URL = "wss://clearnet.yellow.com/ws";
var Client = class {
  constructor(options) {
    this.ws = null;
    ...
    this.options = {
      ...
      backoff: {
        initialDelayMs: options?.backoff?.initialDelayMs ?? 1e3,
        maxDelayMs: options?.backoff?.maxDelayMs ?? 3e4
      }
    };
    this.options.nitrolite = options?.nitrolite;
    if (options?.nitrolite) {
      this.nitroliteClient = new NitroliteClient(options.nitrolite);
    }
    this.builder = new WebsocketBuilder(this.url).withBackoff(
    ...
    try {
      if (typeof globalThis.WebSocket === "undefined") {
        try {
          ...
          globalThis.WebSocket = wsMod.WebSocket ?? wsMod.default ?? wsMod;
        } catch {
        }
      }
   ...
   */
  listen(eventOrCallback, callback) {
    let event;
    let cb;
   ...
   */
  async depositAndCreateChannel(depositAmount, params) {
    if (!this.nitroliteClient) {
      throw new Error("Nitrolite client not configured. Provide nitrolite config in ClientOptions.");
    }
  ...
  async request(request) {
    ...
    const result = new Promise((resolve, reject) => {
      const timer = this.options.requestTimeoutMs > 0 ? setTimeout(() => {
        this.pendingById.delete(id);
  ...
  async sendMessage(message) {
    ...
    if (typeof id === "number" || typeof id === "string") {
      ...
      const data2 = typeof message === "string" ? message : JSON.stringify(message);
      this.ws.send(data2);
      return result;
    }
  ...
  handleMessage(data) {
    ...
    var parsedRpcMethod;
    try {
      response = nitrolite.parseAnyRPCResponse(data);
    ...
    try {
      for (const listener of this.listeners) {
        try {
          if (!listener.event || parsedRpcMethod?.type === listener.event || parsedRpcMethod?.event === listener.event) {
      ...
      for (const listener of this.listeners) {
        try {
          if (!listener.event) {
            const response2 = nitrolite.parseAnyRPCResponse(JSON.stringify(parsed));
            listener.callback(response2);
          }
        } catch (error) {
          console.warn("Listener error:", error);
        }
